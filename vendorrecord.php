...
<div class="container" style=" margin-top:-10px">
    <div class="row">
        <div class="col-sm-10">
             <h4 class="pull-left"> vendor's record </h4>
        </div>
        <div class="col-sm-2">
          <form class="form-inline">
              <input class="search-box pull-right form-control " id="veName" type="search" name="search" placeholder="search"
              style="padding-right:35px" >
       
             <button class= "btn btn-default  glyphicon glyphicon-search" type="submit" style="background: none;float: right; margin-top: -35px;  " ></button> 
...
<div class="table-responsive">
 <table class="table">
        ...
        $sql ="SELECT `newVendor_number`,`firstName`,`lastName`,`companyName`,`mobile`,`billingAddress`,`billingFax` from `vendor_contacts` WHERE `registered_user`='".$current_user."'";
        $result=  mysqli_query($connect,$sql);

        if($result->num_rows >0){
          while($row = $result -> fetch_assoc()){
            echo "<tr><td>" . $row["newVendor_number"] . "</td>" .
